...
class ExtrairFeatures:
	...
	def montarDataframeTodosOsAudios(self, diretorio, freqAmostragem, frameLength, overlapLength):
		...
		for l in np.arange(0,135):
		    aaa.append(str(l))
		colunasTop = ['nomeArquivo', 'ruido', 'pitch', 'tempo', 'velocidade'] + aaa + ['classificacaoCorreta']
	...
	def escalonarFeatures(self):
		...
		dataframeGeral = dataframeGeral.drop(['nomeArquivo', 'velocidade', 'tempo', 'pitch', 'ruido', 'classificacaoCorreta'], axis=1).to_numpy()
